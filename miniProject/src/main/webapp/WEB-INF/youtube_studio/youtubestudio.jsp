...
<body>
	...
	document.getElementById("open").addEventListener("click", function() {
		  var currentChannelId = getChannelIdFromCurrentUrl(); // 현재 페이지 URL에서 channel_id 값을 가져옴
		  var newUrl = "/miniProject/user/uploadvideoForm?channel_id=" + currentChannelId; // 업로드 페이지 URL 생성
		  window.open(newUrl, "new_window", "width=1000,height=800,top=50%,left=50%,scrollbars=yes");  // 새로운 창에서 URL 열기
		});

		function getChannelIdFromCurrentUrl() {
		  var urlParams = new URLSearchParams(window.location.search); // 현재 페이지 URL에서 query string 파싱
		  return urlParams.get("channel_id"); // channel_id 값 반환
		}
